  ...
  return (
    <div className="w-52 border-r border-[var(--border)] bg-white flex flex-col shrink-0 overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[var(--border)]">
        ...
        <span className="text-xs font-semibold text-[var(--text-muted)]">레이어</span>
      </div>

      <div className="flex-1 overflow-y-auto py-1">
        {currentSlide?.components.length === 0 && (
          <p className="text-xs text-[var(--text-disabled)] px-3 py-4 text-center">컴포넌트 없음</p>
        )}
        {currentSlide?.components.map((comp) => (
          <button
            ...
            className={cn(
              'w-full flex items-center gap-2 px-3 py-2 text-xs text-left transition-colors cursor-pointer rounded-[6px] mx-1',
