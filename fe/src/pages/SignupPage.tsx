        ...
        <AuthCard title="회원가입" subtitle="새 계정을 만들어 시작하세요">
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-[var(--text-muted)]">이름</label>
              <Input
                type="text"
                placeholder="홍길동"
                value={name}
          ...
          {/* API Key 안내 */}
          <div className="rounded-[8px] border border-[var(--accent)]/20 bg-[var(--accent-subtle)] px-3 py-2.5">
            <p className="text-xs text-[var(--accent-text)] leading-relaxed">
              가입 후 Anthropic API Key를 등록하면 AI Agent 기능을 사용할 수 있어요.{' '}
              <span className="underline cursor-pointer">한도 설정 방법 보기 →</span>
            </p>
          </div>

          <p className="text-xs text-center text-[var(--text-muted)]">
            이미 계정이 있으신가요?{' '}
            <Link to="/login" className="text-[var(--accent)] hover:text-[var(--accent-hover)] transition-colors">
              로그인
            </Link>
          </p>
        </AuthCard>
