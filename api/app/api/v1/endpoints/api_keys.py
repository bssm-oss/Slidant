...
async def list_api_keys(current_user: CurrentUser, uow: UoW):
    return await api_key_service.list_api_keys(uow, current_user.id)


@router.post("", response_model=ApiKeyResponse, status_code=status.HTTP_201_CREATED)
async def register_api_key(body: ApiKeyCreate, current_user: CurrentUser, uow: UoW):
    return await api_key_service.register_api_key(uow, current_user.id, body.api_key, body.provider)


@router.delete("/{key_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_api_key(key_id: UUID, current_user: CurrentUser, uow: UoW):
    await api_key_service.delete_api_key(uow, current_user.id, key_id)
...
async def get_usage(key_id: UUID, current_user: CurrentUser, uow: UoW):
    return await api_key_service.get_usage(uow, current_user.id, key_id)
