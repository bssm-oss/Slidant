...
def build_agent_graph(
    ...
    # ── html_editor: 기존 슬라이드 HTML 수정 (edit 모드 전용) ────────────────────
    async def html_editor_node(state: AgentState) -> AgentState:
        if on_event:
            on_event("node_start", "✏️ 슬라이드 수정 중...")

        existing_html = state.get("slide_context", "")
        if not existing_html:
            # 기존 HTML 없으면 create 모드로 fallback
        ...
        human_text = (
            ...
            f"MODIFICATION INSTRUCTION: {command}\n"
    ...
    async def slide_deleter_node(state: AgentState) -> AgentState:
        if on_event:
            on_event("node_start", "🗑️ 슬라이드 삭제 중...")
            on_event("step_done", "delete")
            on_event("node_done", "✅ 슬라이드 삭제")
