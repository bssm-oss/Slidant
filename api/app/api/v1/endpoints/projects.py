...
async def list_projects(current_user: CurrentUser, uow: UoW):
    return await project_service.list_projects(uow, current_user.id)


@router.post("", response_model=ProjectResponse, status_code=status.HTTP_201_CREATED)
async def create_project(body: ProjectCreate, current_user: CurrentUser, uow: UoW):
    return await project_service.create_project(uow, current_user.id, body.title)


@router.get("/{project_id}", response_model=ProjectResponse)
async def get_project(project_id: UUID, current_user: CurrentUser, uow: UoW):
    return await project_service.get_project_or_404(uow, project_id, current_user.id)


@router.patch("/{project_id}", response_model=ProjectResponse)
async def update_project(project_id: UUID, body: ProjectUpdate, current_user: CurrentUser, uow: UoW):
    return await project_service.update_project_title(uow, project_id, current_user.id, body.title)


@router.delete("/{project_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_project(project_id: UUID, current_user: CurrentUser, uow: UoW):
    await project_service.delete_project(uow, project_id, current_user.id)
...
async def list_slides(project_id: UUID, current_user: CurrentUser, uow: UoW):
    return await project_service.list_slides(uow, project_id, current_user.id)
...
async def create_slide(project_id: UUID, body: SlideCreate, current_user: CurrentUser, uow: UoW):
    return await project_service.create_slide(uow, project_id, current_user.id, body.title)
...
async def delete_slide(project_id: UUID, slide_id: UUID, current_user: CurrentUser, uow: UoW):
    await project_service.delete_slide(uow, project_id, current_user.id, slide_id)
...
async def reorder_slides(project_id: UUID, body: SlideReorder, current_user: CurrentUser, uow: UoW):
    await project_service.reorder_slides(uow, project_id, current_user.id, body.slide_ids)
...
async def list_components(project_id: UUID, slide_id: UUID, current_user: CurrentUser, uow: UoW):
    return await project_service.list_components(uow, project_id, current_user.id, slide_id)
...
async def create_component(project_id: UUID, slide_id: UUID, body: ComponentCreate, current_user: CurrentUser, uow: UoW):
    return await project_service.create_component(
        uow, project_id, current_user.id, slide_id,
...
async def update_component(project_id: UUID, slide_id: UUID, component_id: UUID, body: ComponentUpdate, current_user: CurrentUser, uow: UoW):
    return await project_service.update_component(
        uow, project_id, current_user.id, slide_id, component_id,
...
async def apply_json_patch(project_id: UUID, slide_id: UUID, body: ComponentPatchRequest, current_user: CurrentUser, uow: UoW):
    return await project_service.apply_json_patch(uow, project_id, current_user.id, slide_id, body.ops)
...
async def delete_component(project_id: UUID, slide_id: UUID, component_id: UUID, current_user: CurrentUser, uow: UoW):
    await project_service.delete_component(uow, project_id, current_user.id, slide_id, component_id)
